...
5. **Mirrors the public API in `frontend/src/routes/api/v1/`** (key auth, `/api/v1/me|actions|contacts|deals|tickets|coach|…`). When you add a new public route, add the matching CLI command in the same PR so the surface stays in sync; if it isn't worth a named command yet, document the `raw` invocation in the CLI README.
6. **Surface parity with the in-app Coach.** `crema coach "<prompt>"` hits the synchronous `/api/v1/coach/chat` proxy, which mints a short-lived rep JWT and forwards to the same `RepAgent` DO the WebSocket chat uses. Same system prompt, same persona overlay, same 17-tool catalog, same 10-step budget — just one-shot JSON instead of a stream. The call is stateless and does not pollute the rep's in-app chat thread; callers pass prior turns via `--history <path>` for multi-turn use.
